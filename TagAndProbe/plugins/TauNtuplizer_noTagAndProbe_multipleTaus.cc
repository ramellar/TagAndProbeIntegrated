#ifndef TAUNTUPLIZER_NOTAGANDPROBE_MULTIPLETAUS_H
#define TAUNTUPLIZER_NOTAGANDPROBE_MULTIPLETAUS_H
...
public:
  ...
  /// Destructor
  virtual ~TauNtuplizer_noTagAndProbe_multipleTaus();
...
TauNtuplizer_noTagAndProbe_multipleTaus::TauNtuplizer_noTagAndProbe_multipleTaus(const edm::ParameterSet& iConfig) :
...
TauNtuplizer_noTagAndProbe_multipleTaus::~TauNtuplizer_noTagAndProbe_multipleTaus()
{}

void TauNtuplizer_noTagAndProbe_multipleTaus::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup)
{
...
void TauNtuplizer_noTagAndProbe_multipleTaus::Initialize() {
  this -> _indexevents = 0;
  this -> _runNumber = 0;
  this -> _lumi = 0;
  this -> _MC_weight = 1;
...
void TauNtuplizer_noTagAndProbe_multipleTaus::beginJob()
{
  edm::Service<TFileService> fs;
...
void TauNtuplizer_noTagAndProbe_multipleTaus::endJob()
{
  return;
}


void TauNtuplizer_noTagAndProbe_multipleTaus::endRun(edm::Run const& iRun, edm::EventSetup const& iSetup)
{
  return;
}


void TauNtuplizer_noTagAndProbe_multipleTaus::analyze(const edm::Event& iEvent, const edm::EventSetup& eSetup)
{
  this -> Initialize();
...
bool TauNtuplizer_noTagAndProbe_multipleTaus::hasFilters(const pat::TriggerObjectStandAlone&  obj , const std::vector<std::string>& filtersToLookFor) {
...
int TauNtuplizer_noTagAndProbe_multipleTaus::FillJet(const edm::View<pat::Jet> *jets, const edm::Event& event){//, JetCorrectionUncertainty* jecUnc){
  int nJets=0;
...
DEFINE_FWK_MODULE(TauNtuplizer_noTagAndProbe_multipleTaus);

#endif //TAUNTUPLIZER_NOTAGANDPROBE_MULTIPLETAUS_H
